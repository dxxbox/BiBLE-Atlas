...
export async function runRecallPipeline(opts: {
  ...
  if (!query) return { hits: [], rendered: "", warnings: [] };
  const warnings: string[] = [];
  const tasks: Array<Promise<RecallHit[]>> = [];
  if (opts.config.enableMemoryRecall) {
    tasks.push(searchDomain("memory", () => opts.runtime.searchMemory({ query, topK: opts.config.recallTopK, minScore: opts.config.recallMinScore, searchType: "hybrid" }), warnings));
  }
  if (opts.config.enableSkillRecall) {
    tasks.push(searchDomain("skill", () => opts.runtime.searchSkill({ query, topK: opts.config.recallTopK, minScore: opts.config.recallMinScore, searchType: "hybrid" }), warnings));
  }
  if (opts.config.enableKnowledgeRecall) {
    for (const tag of opts.config.knowledgeTags) {
      tasks.push(searchDomain("knowledge", () => opts.runtime.searchKnowledge({ query, tag, topK: opts.config.recallTopK, minScore: opts.config.recallMinScore, searchType: "hybrid" }), warnings, tag));
    }
  }
  if (tasks.length === 0) return { hits: [], rendered: "", warnings };
  ...
  return { hits: ranked, rendered: renderRelevantMemories(ranked, budget), warnings };
}
...
async function searchDomain(domain: "memory" | "skill" | "knowledge", fn: () => Promise<Record<string, unknown>>, warnings: string[], tag?: string): Promise<RecallHit[]> {
  try {
    return normalizeHits(domain, await fn(), tag);
  } catch (err) {
