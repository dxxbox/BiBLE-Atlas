...
def _patch_db_writer(writer=None):
    ...
    with patch(
        "bible.features.import.memory_import.storage.store_memory.DatabaseFactory",
...
def _make_payload(**kwargs) -> MemoryImportPayload:
    defaults = {
        "kb_index": "test_kb",
        "tag": "memory",
        "vector_model": None,
    ...
    return MemoryImportPayload(**defaults)
...
def reset_globals():
    ...
    import importlib as _il; container_mod = _il.import_module('bible.features.import.container')
...
def api_client(tmp_path):
    ...
    from bible.features import ImportTaskExecutor
    import bible.features.async_task.container as task_container_mod
    ...
    memory_svc = MemoryImportService(
        store_memory=store,
        ast_guard=ast_guard,
    ...
    executor = ImportTaskExecutor(memory_import_service=memory_svc)
...
def client_with_mock_service(mock_task_service, tmp_path):
    ...
    with patch("bible.api.import.memory_import_api.get_task_service", return_value=mock_task_service):
        with patch("bible.api.import.memory_import_api._get_config", return_value=mock_config):
            app = create_app()
...
class TestParserScriptSelectionChain:
    """MemoryImportService._select_parser_script priority chain."""
...
class TestGetImportTaskEndpoint:
    def test_get_task_not_found_returns_404(self, client_with_mock_service):
        ...
        with patch("bible.api.import.memory_import_api.get_task_repository", return_value=mock_repo):
            from bible.main import create_app
            app = create_app()
    ...
    def test_get_task_found_returns_task_data(self, tmp_path):
        ...
        with patch("bible.api.import.memory_import_api.get_task_repository", return_value=mock_repo):
            with patch("bible.api.import.memory_import_api.get_task_service") as mock_svc:
