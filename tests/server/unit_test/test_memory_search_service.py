...
class TestBindingLookup:
    ...
    def test_binding_lookup_can_use_explicit_kb_index(self) -> None:
        svc, db_writer, _, _ = make_service()
        _search(svc, kb_index=KB_INDEX)
        assert db_writer.binding_index_calls == [("MEMORY", KB_INDEX)]
...
class TestSearcherDelegation:
    ...
    def test_searcher_receives_correct_kb_index(self) -> None:
        svc, _, _, searcher = make_service()
        _search(svc)
        assert searcher.calls[0]["kb_index"] == f"memory_{KB_INDEX}"
...
class TestResponseStructure:
    ...
    def test_kb_index_from_binding(self) -> None:
