"""Integration tests for the full SKILL search path.

Chain under test (real code, no mocks except at the infrastructure boundary)
-----------------------------------------------------------------------------
HTTP POST /api/search/skill
  → FastAPI route    (SkillSearchRequest validation + _validate_tag)
  → SkillSearchService   (param normalisation, binding lookup, model check)
  → SkillSearcher        (vector embedding gate, DSL compilation, hit mapping)
...
from bible.features.upload.skill_upload.parsers.skill_parser.search_profile_builder import (
...
# Complete SKILL profile (hybrid must include vector_field / num_candidates / fields)
    ...
    "hits": [
        {
            ...
            "_source": {
                ...
                "chunk_id": "skill_001::0",        # must be stripped
...
BASE_BODY: dict[str, Any] = {"query": "k8s log cleaner", "tag": "skill"}
...
class TestSuccessResponseShape:
    def test_200_status(self) -> None:
        ...
        assert client.post("/api/search/skill", json=BASE_BODY).status_code == 200
    ...
    def test_flat_upload_profile_still_returns_compact_search_results(self) -> None:
        ...
        binding = IndexBinding(
            ...
            parser_script_sha256="",
            vector_model=VECTOR_MODEL,
            search_profile_json=build_search_profile({}),
            search_profile_sha256="",
            is_active=True,
        )
...
class TestAllSearchTypes:
    ...
    def test_non_vector_types_do_not_call_vector_tool(self, search_type: str) -> None:
        ...
        resp = client.post("/api/search/skill", json={**BASE_BODY, "search_type": search_type})
        assert resp.status_code == 200
        assert fake_vec.ensure_calls == []
        assert fake_vec.embed_calls == []

    def test_keyword_dsl_uses_term_for_single_field(self) -> None:
        """SKILL keyword has one term_field (name.keyword) → direct term query."""
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "search_type": "keyword"})
    ...
    def test_title_dsl_uses_match_on_name(self) -> None:
        """title maps to match on 'name' field for SKILL."""
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "search_type": "title"})
        dsl = fake_db.search_calls[0]["dsl"]
    ...
    def test_text_dsl_uses_multi_match(self) -> None:
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "search_type": "text"})
        dsl = fake_db.search_calls[0]["dsl"]
        assert "multi_match" in dsl["query"]

    def test_text_dsl_includes_skill_specific_fields(self) -> None:
        """text search must target name / description / body / content."""
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "search_type": "text"})
        fields_str = str(fake_db.search_calls[0]["dsl"]["query"]["multi_match"]["fields"])
        assert "description" in fields_str
        assert "body" in fields_str

    def test_vector_calls_ensure_and_embed(self) -> None:
        client, _, fake_vec = make_client()
        resp = client.post("/api/search/skill", json={**BASE_BODY, "search_type": "vector"})
        assert resp.status_code == 200
        assert fake_vec.ensure_calls == [VECTOR_MODEL]
        assert fake_vec.embed_calls == [("k8s log cleaner", VECTOR_MODEL)]

    def test_vector_dsl_uses_knn(self) -> None:
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "search_type": "vector"})
        dsl = fake_db.search_calls[0]["dsl"]
        assert "knn" in dsl["query"]

    def test_hybrid_calls_vector_tool(self) -> None:
        client, _, fake_vec = make_client()
        resp = client.post("/api/search/skill", json={**BASE_BODY, "search_type": "hybrid"})
        assert resp.status_code == 200
        assert fake_vec.embed_calls == [("k8s log cleaner", VECTOR_MODEL)]

    def test_hybrid_dsl_has_two_function_score_arms(self) -> None:
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "search_type": "hybrid"})
        dsl = fake_db.search_calls[0]["dsl"]
    ...
    def test_default_search_type_is_text(self) -> None:
        ...
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json=BASE_BODY)
        dsl = fake_db.search_calls[0]["dsl"]
        assert "multi_match" in dsl["query"]
...
class TestParamNormalisation:
    ...
    def test_top_k_passed_through(self) -> None:
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "top_k": 3})
    ...
    def test_top_k_clamped_to_max_at_service(self) -> None:
        ...
        client, fake_db, _ = make_client(search_cfg=cfg)
        client.post("/api/search/skill", json={**BASE_BODY, "top_k": 20})
        assert fake_db.search_calls[0]["dsl"]["size"] == 20

    def test_vector_weight_from_profile_default_when_omitted(self) -> None:
        """profile hybrid.default_vector_weight = 0.5 must reach the DSL."""
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "search_type": "hybrid"})
        dsl = fake_db.search_calls[0]["dsl"]
        should = dsl["query"]["bool"]["should"]
    ...
    def test_index_used_is_binding_kb_index_directly(self) -> None:
        """SKILL passes binding.kb_index directly (no prefix, unlike MEMORY)."""
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json=BASE_BODY)
...
class TestInputValidation:
    ...
    def test_empty_query_422(self, query: str) -> None:
        client, _, _ = make_client()
        resp = client.post("/api/search/skill", json={"query": query, "tag": "skill"})
        assert resp.status_code == 422

    @pytest.mark.parametrize("tag", ["", "   "])
    def test_empty_tag_422(self, tag: str) -> None:
        client, _, _ = make_client()
        resp = client.post("/api/search/skill", json={"query": "q", "tag": tag})
        assert resp.status_code == 422

    @pytest.mark.parametrize("tag", ["memory", "knowledge_base", "Skill", "SKILL"])
    ...
    def test_top_k_zero_422(self) -> None:
        client, _, _ = make_client()
        resp = client.post("/api/search/skill", json={**BASE_BODY, "top_k": 0})
        assert resp.status_code == 422

    def test_top_k_exceeds_max_400(self) -> None:
        cfg = SearchConfig(max_top_k=20)
        client, _, _ = make_client(search_cfg=cfg)
        resp = client.post("/api/search/skill", json={**BASE_BODY, "top_k": 999})
        assert resp.status_code == 400
        assert resp.json()["detail"]["code"] == "INVALID_ARGUMENT"

    def test_vector_weight_out_of_range_422(self) -> None:
        client, _, _ = make_client()
        resp = client.post("/api/search/skill", json={**BASE_BODY, "vector_weight": 1.5})
        assert resp.status_code == 422

    def test_unknown_search_type_400(self) -> None:
        client, _, _ = make_client()
        resp = client.post("/api/search/skill", json={**BASE_BODY, "search_type": "bm25_fuzzy"})
        assert resp.status_code == 400
...
class TestBusinessErrors:
    def test_binding_not_found_404(self) -> None:
        ...
        resp = client.post("/api/search/skill", json=BASE_BODY)
        assert resp.status_code == 404
        assert resp.json()["detail"]["code"] == "INDEX_NOT_BOUND"

    def test_vector_model_conflict_409(self) -> None:
        client, _, _ = make_client()
        resp = client.post("/api/search/skill", json={**BASE_BODY, "vector_model": "other/model"})
    ...
    def test_bad_profile_keyword_empty_term_fields_422(self) -> None:
        ...
        bad_binding = IndexBinding(
            domain_type="SKILL", kb_index=KB_INDEX, tag="skill",
        ...
        resp = client.post("/api/search/skill", json={**BASE_BODY, "search_type": "keyword"})
        assert resp.status_code == 422
    ...
    def test_missing_search_type_in_profile_422(self) -> None:
        ...
        client, _, _ = make_client(db_writer=FakeDBWriter(binding=binding))
        resp = client.post("/api/search/skill", json={**BASE_BODY, "search_type": "text"})
        assert resp.status_code == 422
        assert resp.json()["detail"]["code"] == "SEARCH_PROFILE_INVALID"
    ...
    def test_vector_embed_exception_500(self) -> None:
        ...
        resp = client.post(
            "/api/search/skill", json={**BASE_BODY, "search_type": "vector"}
        )
        assert resp.status_code == 500
        assert resp.json()["detail"]["code"] == "INTERNAL_ERROR"
...
class TestDslCorrectness:
    def test_keyword_term_value_equals_query(self) -> None:
        ...
        client.post(
            "/api/search/skill",
            json={**BASE_BODY, "query": "k8s-log-cleaner", "search_type": "keyword"},
        )
        dsl = fake_db.search_calls[0]["dsl"]
        term_value = dsl["query"]["term"]["name.keyword"]["value"]
        assert term_value == "k8s-log-cleaner"

    def test_text_multi_match_contains_query(self) -> None:
        client, fake_db, _ = make_client()
        client.post("/api/search/skill", json={**BASE_BODY, "query": "oom killer guard", "search_type": "text"})
        dsl = fake_db.search_calls[0]["dsl"]
        assert dsl["query"]["multi_match"]["query"] == "oom killer guard"
...
# ──────────────────────────────────────────────────────────────────────────────
# 7. related_storage_paths end-to-end
# ──────────────────────────────────────────────────────────────────────────────
...
class TestRelatedStoragePaths:
    ...
    def test_missing_metadata_field_does_not_raise(self) -> None:
        ...
        hits = {
            "total": 1,
            "hits": [{"_score": 0.5, "_source": {"doc_id": "x", "name": "bare-skill"}}],
        }
        client, _, _ = make_client(db_writer=FakeDBWriter(hits=hits))
        resp = client.post("/api/search/skill", json=BASE_BODY)
        assert resp.status_code == 200
        item = resp.json()["results"]["skill"][0]
        assert "related_storage_paths" not in item
