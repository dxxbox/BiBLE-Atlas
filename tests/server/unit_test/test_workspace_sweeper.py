...
from bible.features.upload.workspace_sweeper import WorkspaceSweeper
...
class TestWorkspaceSweeperSweep:
    ...
    def test_sweep_logs_deleted_count_when_nonzero(self, caplog):
        ...
        sweeper_logger = logging.getLogger("bible.features.upload.workspace_sweeper")
        sweeper_logger.propagate = True
        try:
            with caplog.at_level(logging.INFO, logger="bible.features.upload.workspace_sweeper"):
                sweeper._sweep_once()
        finally:
    ...
    def test_sweep_exception_is_swallowed(self):
        ...
        with patch("bible.features.upload.workspace_sweeper.logger") as mock_logger:
