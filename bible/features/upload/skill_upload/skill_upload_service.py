...
from .storage.store_skill import StoreSkill
from ..types import SkillImportPayload, ParseResult
...
class SkillImportService:
    def __init__(
        self,
        store_skill: StoreSkill,
        ast_guard: ASTGuard,
        sandbox_runner: SandboxRunner,
        parsers_dir: str,
        config: Any = None,
    ) -> None:
        self._store_skill = store_skill
        self._ast_guard = ast_guard
        self._sandbox_runner = sandbox_runner
        self._parsers_dir = parsers_dir
        self._custom_parsers_dir = self._resolve_custom_parsers_dir(config, parsers_dir)
        self._config = config
    ...
    def execute_task(
        ...
        if self._config is not None:
            keep_failed = (
                self._config.import_skill.keep_failed_workspace
                if hasattr(self._config, "import_skill")
                else False
            )

        logger.info(
            "[task=%s] Starting skill import: kb_index=%s files=%d vector_model=%s",
        ...
        try:
            ...
            parser_script_source = self._parser_script_source_name(
                script_path,
                user_supplied=_user_supplied,
            )
            ...
            result = self._store_skill.store(
            ...
            self._persist_custom_parser_if_needed(
                script_path,
                user_supplied=_user_supplied,
                task_id=task_id,
            )
    ...
    def save_uploaded_parser(
        self,
        parser_script: Any,
        parsers_dir: str,
        target_name: str = "parse_skill.py",
    ) -> str:
        os.makedirs(parsers_dir, exist_ok=True)
        dest = os.path.join(parsers_dir, target_name)
        if hasattr(parser_script, "file"):
            parser_script.file.seek(0)
            content = parser_script.file.read()
        elif isinstance(parser_script, (bytes, bytearray)):
            content = bytes(parser_script)
        else:
            content = parser_script
    ...
    def cleanup_staged_workspace(
        ...
        self._store_skill.cleanup_task_workspace(task_id, keep_failed=keep_failed)
    ...
    def _select_parser_script(self, payload: SkillImportPayload, task_id: str) -> str:
        ...
        if payload.parser_context and "script_content" in payload.parser_context:
            ...
            base_dir = payload.session_upload_dir or os.path.join(self._store_skill._import_work_root, task_id)
        ...
        candidate_custom = os.path.join(self._custom_parsers_dir, "parse_skill.py")
        if os.path.exists(candidate_custom):
            return candidate_custom

        candidate_skill = os.path.join(self._parsers_dir, "parse_skill.py")
        if os.path.exists(candidate_skill):
        ...
        raise DomainError(
            ErrorCode.NOT_FOUND,
            "No parser script found; upload a parser script, embed it in parser_context.script_content, or pre-register parse_skill.py",
            details={"code": "PARSER_SCRIPT_NOT_FOUND"},
        )

    @staticmethod
    def _resolve_custom_parsers_dir(config: Any, parsers_dir: str) -> str:
        if config is not None and hasattr(config, "import_skill"):
            custom_dir = getattr(config.import_skill, "custom_parsers_dir", None)
            if custom_dir:
                return custom_dir
        return os.path.join(parsers_dir, "custom")

    @staticmethod
    def _parser_script_source_name(script_path: str, *, user_supplied: bool) -> str:
        return "parse_skill.py" if user_supplied else os.path.basename(script_path)

    def _persist_custom_parser_if_needed(
        self,
        script_path: str,
        *,
        user_supplied: bool,
        ...
        os.makedirs(self._custom_parsers_dir, exist_ok=True)
        target_path = os.path.join(self._custom_parsers_dir, "parse_skill.py")
        fd, tmp_path = tempfile.mkstemp(
            prefix=".parse_skill.",
            suffix=".tmp",
            dir=self._custom_parsers_dir,
        )
        try:
            ...
            logger.info(
                "[task=%s] Persisted uploaded SKILL parser to %s",
                task_id,
                target_path,
            )
        finally:
    ...
    def _is_custom_parser(self, script_path: str) -> bool:
        return os.path.abspath(script_path) == os.path.abspath(
            os.path.join(self._custom_parsers_dir, "parse_skill.py")
        )
