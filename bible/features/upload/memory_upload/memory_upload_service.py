...
from ..types import MemoryUploadPayload, ParseResult
...
class MemoryUploadService:
    ...
    def _select_parser_script(self, payload: MemoryUploadPayload, task_id: str) -> str:
        ...
        candidate_custom = os.path.join(self._custom_parsers_dir, "parse_memory.py")
    ...
    def _resolve_custom_parsers_dir(config: Any, parsers_dir: str) -> str:
        if config is not None and hasattr(config, "import_memory"):
            custom_dir = getattr(config.import_memory, "custom_parsers_dir", None)
            if custom_dir:
                return custom_dir
    ...
    def _parser_script_source_name(script_path: str, *, user_supplied: bool) -> str:
        return "parse_memory.py" if user_supplied else os.path.basename(script_path)
    ...
    def _persist_custom_parser_if_needed(
        ...
        *,
        ...
        target_path = os.path.join(self._custom_parsers_dir, "parse_memory.py")
        ...
        try:
            ...
            logger.info(
                "[task=%s] Persisted uploaded MEMORY parser to %s",
                task_id,
                target_path,
            )
        finally:
    ...
    def _is_custom_parser(self, script_path: str) -> bool:
        return os.path.abspath(script_path) == os.path.abspath(
            os.path.join(self._custom_parsers_dir, "parse_memory.py")
        )
