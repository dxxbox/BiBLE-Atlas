...
class StoreMemory:
    ...
    def store(
        ...
        raw_search_profile: dict[str, Any] = dict(parse_result.search_profile)
        source_template = self._extract_vector_source_template(raw_search_profile)
        vector_options = self._extract_vector_options(raw_search_profile)
        search_profile = self._search_profile_for_binding(raw_search_profile)
        ...
        chunks = self._vectorize_if_needed(
            chunks,
            vector_model,
            search_profile,
        ...
        store_result = self._store_parsed_content(
            kb_index,
            chunks,
    ...
    def _vectorize_if_needed(
        ...
        *,
        ...
        source_template = source_template or self._extract_vector_source_template(search_profile)
        try:
    ...
    @staticmethod
    def _extract_vector_source_template(search_profile: dict[str, Any]) -> str | None:
        source_template = search_profile.get("source_template")
        if isinstance(source_template, str):
            return source_template

        type_profile = search_profile.get("search_type_profile")
        if isinstance(type_profile, dict):
            vector_profile = type_profile.get("vector")
            if isinstance(vector_profile, dict):
                nested_template = vector_profile.get("source_template")
                if isinstance(nested_template, str):
                    return nested_template
        return None

    @staticmethod
    def _search_profile_for_binding(search_profile: dict[str, Any]) -> dict[str, Any]:
        binding_profile = deepcopy(search_profile)
        binding_profile.pop("source_template", None)

        type_profile = binding_profile.get("search_type_profile")
        if isinstance(type_profile, dict):
            vector_profile = type_profile.get("vector")
            if isinstance(vector_profile, dict):
                vector_profile.pop("source_template", None)
        return binding_profile

    @staticmethod
    def _extract_vector_options(search_profile: dict[str, Any]) -> dict[str, Any] | None:
        type_profile = search_profile.get("search_type_profile")
        if not isinstance(type_profile, dict):
            return None

        vector_profile = type_profile.get("vector")
        if not isinstance(vector_profile, dict):
            return None

        options: dict[str, Any] = {}
        num_candidates = vector_profile.get("num_candidates")
    ...
    def _store_parsed_content(
        ...
        *,
        ...
        return writer.bulk_upsert_content_docs(
            index,
            chunks,
            vector_options=vector_options,
        )
