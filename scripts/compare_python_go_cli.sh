...
# Python CLI 已废弃（bible_cli_DEPRECATED），此脚本作为历史迁移证据归档，不再作为持续门禁。
# Go CLI 的契约回归请使用 bible_cli_go 目录内的 go test ./... 及 testdata/golden。
# PY_CMD=(uv run python -m bible_cli_DEPRECATED.python_cli)
...
echo "警告：Python CLI 已废弃，此脚本仅作历史归档用途，Python 侧对比已停用。" >&2
echo "如需 Go CLI 契约回归，请执行：cd bible_cli_go && go test ./..." >&2
exit 0
